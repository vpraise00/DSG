...
def get_obj_position_from_private(scenario_root):
    """
    Retrieve the LinkPosition (id and index) of the falling object (OBJ)
    from the Private element created in private_storyboard.
    """
    lp = scenario_root.find(".//Private[@entityRef='OBJ']/PrivateAction/TeleportAction/Position/LinkPosition")
    if lp is None:
...
def add_Ego_lanechange_action(scenario_root, distance_input, obj_pos):
    """
...
def add_Ego_stop_action(scenario_root, distance_input, obj_pos):
    """
