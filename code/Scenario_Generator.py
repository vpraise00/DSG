...
def agr_parser():
    ...
    parser.add_argument("--obj_pos", type=str, choices=["relative", "link"], default="relative", help="Object position definition [relative, link]")
    parser.add_argument("--action", type=str, choices=["lanechange", "stop"], default="stop", help="Action type only required in fallOBJ [lanechange, stop]")
...
if __name__ == "__main__":
    ...
    if args.disturbance == "rain":
        ...
        # 선택한 object 방식에 따라 호출
        if obj_module == "relative":
            actions.append(private_storyboard_rel(root))
        else:
